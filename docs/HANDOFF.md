...
- Release checkpoint: SALMAN OS UI/UX workspace baseline. The current local baseline separates dashboard, CRM client management, and client workroom detail screens; keeps the dark navigation-only sidebar with a bright CRM workspace; standardizes visible product naming as SALMAN OS; keeps client detail content behind tabs; and completes mock-first UX cleanup for the 자료실, 업무, and 일정 tabs. The 자료실 tab uses folder/file/detail panels backed by the mock Drive repository, while 업무 and 일정 use scan-friendly operational mock views with prepared actions only. No actual Google Drive API wiring, frontend `/api/drive/*` fetch, Supabase insert/update/delete, Supabase schema/RLS/RPC change, chatbot/RAG/vector work, Playwright automation, business money automation, package change, or real env/credential change has been introduced.
...
Next recommended work: 로그/작업자 기록 UX 정리.

Keep the current SALMAN OS UI/UX workspace baseline intact unless a later task explicitly changes it. Follow-up work should not add actual Google Drive API wiring, frontend `/api/drive/*` fetch, Supabase insert/update/delete, Supabase schema/RLS/RPC changes, chatbot/RAG/vector work, Playwright automation, business money automation, package changes, or real env/credential changes without separate approval.

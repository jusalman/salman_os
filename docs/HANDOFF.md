...
- Resume note: When the user says to continue next week, start from TASK-116 or rerun TASK-115 only after ensuring the required server-only Drive smoke settings are present in the shell environment used by Codex commands. TASK-115 again stopped at `npm.cmd run drive:smoke:gate` because required setting names were missing, so `npm.cmd run drive:smoke:list` was not executed. Actual Drive API calls, credential reads, `.env.local` reads, `.env.example` Google entries, frontend fetch wiring, Supabase schema/RLS/RPC changes, embeddings, vectors, and chatbot UI remain out of scope. Keep README/HANDOFF/AGENTS/README_INDEX as the always-read entry points.
...
- Current write phase: TASK-115 stopped at the gate failure after verification.
...
- Next task: TASK-116 either rerun the approved local Drive list smoke after making the required server-only settings visible to the Codex command shell, or continue implementation without actual Drive API execution.
